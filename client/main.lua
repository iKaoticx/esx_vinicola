...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('vinho_clothes_civil'), value = 'citizen_wear'},
				{label = _U('vinho_clothes_vinho'), value = 'vinicola_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'vinicola_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'vinicola_actions_menu'
...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'vinicola_actions',
		{
			title    = 'Vinícola - Casa Valduga',
			align    = 'top-left',
			elements = elements
		},
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'vinicola', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'vinicola', vehicleProps)
		...
		local elements = {
			{label = 'Caminhonete de Trabalho',  value = 'bison3'},
		}
...
function OpenMobileVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_vinicola_actions',
		{
			title    = 'vinicola',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_vinicola', 'Viticultor', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_viticultorjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_viticultorjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_viticultorjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_viticultorjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_viticultorjob:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola'  then
		CurrentAction     = 'uva_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVinho' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola'  then
		CurrentAction     = 'vinho_traitement'
		CurrentActionMsg  = _U('press_traitementv')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementSuco' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola'  then
		CurrentAction     = 'suco_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'ViticultorActions' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
		CurrentAction     = 'vinicola_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
...
AddEventHandler('esx_viticultorjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
		TriggerServerEvent('esx_viticultorjob:stopHarvest')
	end  
	if (zone == 'TraitementVinho' or zone == 'TraitementSuco') and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
		TriggerServerEvent('esx_viticultorjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
		TriggerServerEvent('esx_viticultorjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.ViticultorActions.Pos.x, Config.Zones.ViticultorActions.Pos.y, Config.Zones.ViticultorActions.Pos.z)
		...
		AddTextComponentString("Vinicola - Casa Valduga")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_viticultorjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_viticultorjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'uva_harvest' then
					TriggerServerEvent('esx_viticultorjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'suco_traitement' then
					TriggerServerEvent('esx_viticultorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vinho_traitement' then
					TriggerServerEvent('esx_viticultorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_viticultorjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'vinicola_actions_menu' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'vinicola', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'vinicola' and (GetGameTimer() - GUI.Time) > 150 then
